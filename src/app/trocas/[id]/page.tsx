...
export default function TrocaDetalhePage() {
  ...
  if (erro) return <AppLayout><div style={{ color: '#E5584A' }}>{erro}</div></AppLayout>
      ...
      <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 1000 }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
            <Info label="Devolvido" valor={BRL(Number(troca.valor_original))} cor="#E5584A" />
            <Info label="Novos" valor={BRL(Number(troca.valor_troca))} cor="#4CAF82" />
            <Info label={diff > 0 ? 'Cliente pagou +' : diff < 0 ? 'Crédito ao cliente' : 'Diferença'} valor={BRL(Math.abs(diff))} cor={diff > 0 ? '#C9A84C' : '#4D9ECC'} destaque />
          </div>
          {troca.observacao && (
        ...
        {devolvidos.length > 0 && (
          <SecaoItens titulo="Devolvidos" cor="#E5584A" itens={devolvidos} />
        )}
        {novos.length > 0 && (
          <SecaoItens titulo="Novos" cor="#4CAF82" itens={novos} />
        )}
      </div>
    </AppLayout>
  )
