...
function EstoqueBadge({ estoque, minimo }: { estoque: number; minimo: number }) {
  ...
  if (estoque <= minimo)      return <span className="badge" style={{ background: 'rgba(245,166,35,0.1)', color: '#f5a623', border: '1px solid rgba(245,166,35,0.2)' }}>Estoque baixo</span>
    ...
    <AppLayout>
      <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 34, fontWeight: 700, color: '#f5ecd7' }}>Produtos</h1>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {GRUPOS.map(g => (
              <button key={g} onClick={() => { setGrupo(g); setPagina(1) }} style={{
                padding: '7px 12px', borderRadius: 8, border: `1px solid ${grupo === g ? 'rgba(212,175,95,0.3)' : 'var(--border)'}`,
                background: grupo === g ? 'rgba(212,175,95,0.18)' : 'rgba(255,255,255,0.03)',
                color: grupo === g ? '#d4af5f' : 'var(--text-muted)',
          ...
          <button onClick={() => { setFiltroBaixo(!filtroBaixo); setPagina(1) }} style={{
            padding: '7px 14px', borderRadius: 8, border: `1px solid ${filtroBaixo ? 'rgba(245,166,35,0.3)' : 'var(--border)'}`,
            background: filtroBaixo ? 'rgba(245,166,35,0.12)' : 'rgba(255,255,255,0.03)',
            color: filtroBaixo ? '#f5a623' : 'var(--text-muted)',
            fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 600, cursor: 'pointer',
        ...
        {vista === 'tabela' && (
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 120px 100px 110px 120px 100px', padding: '12px 20px', borderBottom: '1px solid var(--border)', background: 'rgba(212,175,95,0.03)' }}>
            ...
            {loading ? (
              ...
              <div key={p.id}
                ...
                style={{
                  ...
                  borderBottom: i < produtos.length - 1 ? '1px solid rgba(212,175,95,0.05)' : 'none',
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 13, color: '#f5ecd7', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 260 }}>{p.descricao}</div>
                ...
                <div style={{ fontSize: 14, fontFamily: 'var(--font-display)', fontWeight: 700, color: p.estoque <= 0 ? '#ef6b4d' : p.estoque <= p.estoque_minimo ? '#f5a623' : '#64c88c' }}>
                  {p.estoque}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14 }}>
            {loading ? (
              ...
              <div key={p.id} onClick={() => router.push(`/produtos/${p.id}`)} className="card"
                ...
                onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(212,175,95,0.3)')}
                onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--border)')}
              >
                {/* Placeholder imagem */}
                <div style={{
                  width: '100%', aspectRatio: '1', borderRadius: 10,
                  background: 'linear-gradient(135deg, rgba(212,175,95,0.1), rgba(212,175,95,0.02))',
                  border: '1px solid var(--border)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: 'var(--font-display)', fontSize: 28, color: 'rgba(212,175,95,0.25)',
                }}>◫</div>
                <div style={{ fontSize: 12, color: '#f5ecd7', fontWeight: 600, lineHeight: 1.3, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                  {p.descricao}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, color: '#d4af5f' }}>{BRL(p.preco_venda)}</span>
                  <span style={{ fontSize: 11, color: p.estoque <= p.estoque_minimo ? '#f5a623' : '#64c88c' }}>Est: {p.estoque}</span>
                </div>
              </div>
            ))}
          </div>
